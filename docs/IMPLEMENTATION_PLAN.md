...
**Status (02 Jul):** B1 ✅ · B2 ✅ (RQ1 done, honest-negative streaming) · B3 ⏳ issued (`rq2-serving-spike`) · B4–B7 pending B3 decisions.
...
## Batch 4 — Phase 3: Model compression  `[PROMOTED · primary lever for RQ2 memory AND RQ3 single-request latency]`
...
## Batch 5 — Phase 2b: Aho-Corasick over DAWG  `[REFRAMED → batch-throughput lever · CONDITIONAL on Batch-3 decision]`
Run only if AC/DAWG survives the spike. Value is **batch throughput** (76 µs/dom, feature-bound) + ~23 MiB dict RAM — *not* the RQ2/RQ3 targets.
...
6. Re-measure RQ1 spawn costs under Linux `fork` (Windows spawn ~0.3–0.5 s/worker inflated Batch-2 startup); re-run tl2cgen compiled-`.so` bench if deferred from the spike.
...
| distill → linear/1-tree | single-request + RSS (collapses both) | probe 4 → decision |
...
| AC/DAWG | **batch throughput only** + 23 MiB | Batch 5, conditional |
